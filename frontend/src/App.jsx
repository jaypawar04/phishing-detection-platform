...
function App() {
  ...
  const downloadReport = () => {
    const scan = selectedScan || result;

    if (!scan) {
      alert("Please select or generate a scan result first.");
      return;
    }

    const doc = new jsPDF();
    const reasons = normalizeReasons(scan.reasons);

    doc.setFontSize(20);
    doc.text("Phishing Detection Report", 20, 20);

    doc.setFontSize(10);
    doc.text("Phishing Detection Platform | SOC Security Analysis Engine", 20, 28);
    doc.line(20, 34, 190, 34);

    doc.setFontSize(12);
    doc.text(`Scan ID: ${scan.id ?? "Current Scan"}`, 20, 48);
    doc.text(`URL: ${scan.url || "N/A"}`, 20, 58);
    doc.text(`Verdict: ${formatVerdict(scan.verdict || "N/A")}`, 20, 68);
    doc.text(`Risk Score: ${scan.risk_score ?? "N/A"}/100`, 20, 78);
    doc.text(
      `Scanned At: ${
        scan.scanned_at
    ...
    doc.setFontSize(14);
    doc.text("Detection Reasons", 20, 108);

    doc.setFontSize(10);
    let y = 120;

    if (reasons.length === 0) {
      doc.text("No suspicious indicators detected.", 20, y);
    } else {
      reasons.forEach((reason, index) => {
        const indicator =
          typeof reason === "string"
            ? reason
            : reason.indicator || reason.reason || reason.description || JSON.stringify(reason);

        const lines = doc.splitTextToSize(`${index + 1}. ${indicator}`, 165);
        doc.text(lines, 20, y);
        y += lines.length * 6 + 4;
    ...
    doc.setFontSize(9);
    doc.text("Generated by Phishing Detection Platform", 20, 285);
    doc.save(`phishing-scan-${scan.id || "report"}.pdf`);
  };
  ...
  <a
    href="#scanner"
    className={`sidebar-link ${activeSection === "scanner" ? "active" : ""}`}
  ...
  <a
    href="#alerts"
    className={`sidebar-link ${activeSection === "alerts" ? "active" : ""}`}
  ...
  <a
    href="#history"
    className={`sidebar-link ${activeSection === "history" ? "active" : ""}`}
  ...
  >
    ...
    <span>Reports</span>
      ...
      <main className="container">
        ...
        <section id="reports" className="reports-card">
          <div className="section-heading">
            <div>
              <span className="eyebrow">SECURITY REPORTING</span>
              <h2>Reports & Evidence</h2>
              <p>Export the selected investigation result as a PDF report.</p>
            </div>
          </div>
          ...
          <div className="reports-grid">
            <div className="report-action-card">
              ...
              <div>
                <h3>Phishing Analysis Report</h3>
                <p>
                  Includes the analyzed URL, verdict, risk score, timestamp,
                  and detection indicators.
                </p>
              </div>
              <button className="refresh-button" onClick={downloadReport}>
                Download PDF
              </button>
            </div>

            <div className="report-action-card">
              <span className="report-icon">SOC</span>
              <div>
                <h3>Investigation Workflow</h3>
                <p>
                  Review alerts, open scan details, and retain evidence for
        ...
        <section id="settings" className="settings-card">
          <div className="section-heading">
            <div>
              ...
              <h2>Settings</h2>
              <p>Local configuration and operational information for this lab.</p>
            </div>
          </div>
          ...
          <div className="settings-grid">
            <div className="setting-row">
              <div>
                ...
                <span>FastAPI detection backend</span>
              </div>
              <code>{API_URL}</code>
            </div>
            ...
            <div className="setting-row">
              <div>
                ...
                <span>Rule-based phishing analysis</span>
              </div>
            ...
            <div className="setting-row">
              <div>
                ...
                <span>Local scan history and SOC alerts</span>
              </div>
              <span className="setting-status">Operational</span>
            </div>
          </div>
        </section>
